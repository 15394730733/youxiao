...
//		paint.setColor(Color.rgb(0, 0, 0));// 设置颜色
...
//		paint.setColor(Color.rgb(4, 115, 157));// 设置颜色
	...
	public Drawl(Context context, String psd,List<Point> list,GestureCallBack callBack) {
		super(context);
		paint = new Paint(Paint.DITHER_FLAG);// 创建一个画笔
		bitmap = Bitmap.createBitmap(480, 854, Bitmap.Config.ARGB_8888); // 设置位图的宽高
		canvas = new Canvas();
...
//		paint.setColor(Color.rgb(4, 115, 157));// 设置颜色
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:// 当手指抬起的时候
			...
			if(null==passWord){
				...
				callBack.setPassWord(true);
			}else if(passWord.equals(passWordSb.toString())){
				//代表用户绘制的密码手势与传入的密码相同
				callBack.checkedSuccess(passWord);
	...
	public interface GestureCallBack{
		 ...
		 * 代表用户绘制的密码与传入的密码相同
		 */
		public abstract void checkedSuccess(String psd);
		/**
		 * 代表用户绘制的密码与传入的密码不相同
		 */
		 ...
		 * 设置手势密码
		 */
		public abstract void setPassWord(boolean info);
	}
